...
include_recipe "thethrum_tomcat::default"
...
when 'generic'
  ...
  link "#{node['tomcat']['config_dir']}/apps.conf" do
    to "#{node['tomcat']['home']}/etc/tomcat/apps.conf.#{node['app_environment']}.#{node['application']}"
    not_if { ::File.symlink?("/etc/tomcat/apps.conf") }
  end
when 'pr'
  puts 'managing apps.conf as if it were prod, as this is the PR role'
  link "#{node['tomcat']['config_dir']}/apps.conf" do
    to "#{node['tomcat']['home']}/etc/tomcat/apps.conf.prod.#{node['application']}"
    not_if { ::File.symlink?("/etc/tomcat/apps.conf") }
  end 
else
  link "#{node['tomcat']['config_dir']}/apps.conf" do
